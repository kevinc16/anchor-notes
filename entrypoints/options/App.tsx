...
export default function App() {
  ...
  const [editCoverage, setEditCoverage] = useState<HighlightCoverage>('medium');
  ...
  function startEdit(note: AnchorNote) {
    ...
    setEditCoverage(note.highlightCoverage ?? 'medium');
  }
  ...
  async function persistEdit() {
    ...
    await saveNote(applyLibraryNoteEdits(editing, {
      ...
      summary: editSummary,
      color: editColor,
      highlightCoverage: editCoverage,
      tags: editTags,
    }));
  ...
  async function saveSettings() {
    ...
    if (apiKey) {
      try {
        const aiApiKeyEncrypted = await encryptSecret(apiKey, vaultPassphrase);
        nextSettings = { ...settings, aiApiKey: '', aiApiKeyEncrypted };
      ...
      nextSettings = { ...settings, aiApiKey: '' };
  ...
  async function forgetApiKey() {
    ...
    const nextSettings = { ...settings, aiApiKey: '', aiApiKeyEncrypted: undefined };
            ...
            <div className="mt-7 max-w-[680px] rounded-[17px] border border-line bg-card p-6">
              ...
              {settings.aiEnabled && settings.aiProvider !== 'local' && <>
                ...
                {settings.aiProvider !== 'ollama' && <>
                  <Field
                    label="API key"
                    help={settings.aiApiKeyEncrypted
                      ...
                      : settings.aiApiKey
                        ? 'This legacy key is still plaintext. Enter a passphrase below and save to encrypt it.'
                        : 'The key will be encrypted before it is stored locally.'}
                  >
                  ...
                  <Field label="Encryption passphrase" help={`At least ${MIN_PASSPHRASE_LENGTH} characters. Anchor Notes never stores this passphrase; use it to unlock the key after restarting Chrome.`}>
                    <input className={fieldClass} type="password" autoComplete="off" placeholder="Not stored by Anchor Notes" value={vaultPassphrase} onChange={(event) => setVaultPassphrase(event.target.value)} />
                  </Field>
      ...
      {editing && (
        <div className="fixed inset-0 z-20 grid place-items-center bg-ink/35 p-4 backdrop-blur-sm" role="presentation" onMouseDown={(event) => { if (event.target === event.currentTarget) setEditing(null); }}>
          <section className="max-h-[calc(100vh-2rem)] w-full max-w-[520px] overflow-y-auto rounded-[18px] border border-line bg-paper p-6 shadow-2xl" role="dialog" aria-modal="true" aria-labelledby="edit-title">
            ...
            <Field label="Highlight coverage" help="Controls how much of each highlighted text line is filled."><CoveragePicker value={editCoverage} onChange={setEditCoverage} /></Field>
